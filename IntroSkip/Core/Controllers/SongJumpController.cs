...
namespace SheepIntroSkip.Core
{
    internal class SongJumpController : MonoBehaviour
    {
        ...
        /// </summary>
        internal float NextTime { get; private set; } = 0;

        internal float CannotSkipAnymoreTime { get; private set; } = 0;
        ...
        {
            if (WaitingForSkip == false)
            {
                CurrentSkipTime = 0;
                return;
            }

            if (CP_SDK_BS.Game.Logic.ActiveScene == Logic.ESceneType.Menu) return;
        ...
        public async void SetCanSkip(float time, TextViewController.EValueType anyInformation, bool isMapEnd = false)
        {
            ...
            if (isMapEnd)
            {
                CannotSkipAnymoreTime = time - ISConfig.Instance.BeforeNoteTime;
            }
            else
            {
                CannotSkipAnymoreTime =
                time

                -
#if ONE_DOT_FOUR
                BeatmapHelper.BeatmapHelper.GetTimeFromBeat(ObjectsGrabber.ObjectSpawnController.beatmapObjectSpawnMovementData.startHalfJumpDurationInBeats, ParseBeatmap.BPMs, true)
#else
                    ObjectsGrabber.ObjectSpawnController.beatmapObjectSpawnMovementData.jumpDuration
#endif

                - ISConfig.Instance.BeforeNoteTime;
            }
